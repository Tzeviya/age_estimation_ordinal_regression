...
class kaggle(data.Dataset): 
	...
	def __getitem__(self, idx):
		...
		label = int(self.pic_dataset_dict[example][1])
...
def load_dataset(train_path, val_path, test_path, batch_size, device, small = False):
	...
	if train_path: train_dataset = kaggle(train_path, transform, 'train', small)
	if val_path: val_dataset = kaggle(val_path, transform, 'val', small)
	if test_path: test_dataset = kaggle(test_path, transform, 'test', small)
